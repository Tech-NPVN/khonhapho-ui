...
export interface MeetingMemberTypes {
  id?: string;
  username?: string;
  branch?: string;
  checkin_time?: string;
...
const columns: TableProps<MeetingMemberTypes>['columns'] = [
  {
    title: 'Họ & Tên',
    key: 'username',
    dataIndex: 'username',
    className: 'text-left !py-1 header-name',
    render: (name: MeetingMemberTypes['username']) => (
  ...
  },
  {
    ...
    className: 'text-center',
    render: (checkout_time: MeetingMemberTypes['checkout_time']) => (
      <span className="block text-center w-full">{checkout_time}</span>
    ),
  },
];

const demo: MeetingMemberTypes = {
  id: '1',
  branch: 'Hà Nội',
...
const dataSource: MeetingMemberTypes[] = Array.from({ length: 12 }, () => ({ ...demo }));

export const MeetingMemberTable = () => {
...
type ModalMeetingMemberProps = {
  open?: boolean;
  onClose?: () => void;
};

const MeetingMemberHeader: React.FC<ModalMeetingMemberProps> = () => {
...
const ModalMeetingMember: React.FC<ModalMeetingMemberProps> = ({ open, onClose }) => {
  return (
    <>
      {open && (
        <ModalWithHash
