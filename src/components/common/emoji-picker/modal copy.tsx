...
const chunkSize = 1; // Số nhóm load mỗi lần
...
const EmojiContent: React.FC<EmojiPickerProps> = ({ onSelect }) => {
  ...
  const [visibleGroups, setVisibleGroups] = useState<string[]>(
    Tabs.slice(0, 3).map((tab) => tab.key), // Load sẵn 2 nhóm đầu tiên
  );
  const [currentChunk, setCurrentChunk] = useState(3); // Bắt đầu từ nhóm tiếp theo
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement, UIEvent>) => {
    ...
    // Xác định nhóm gần nhất dựa trên khoảng cách `scrollTop`
    let closestGroup = '';
    ...
    // Đặt tabIndex tương ứng với nhóm gần nhất
    const tabIndex = Tabs.findIndex((tab) => tab.key === closestGroup);
    if (tabIndex !== -1) setTabIndex(tabIndex);

    // Kiểm tra nếu cuộn đến cuối để load thêm nhóm
    const isAtBottom = target.scrollHeight - target.scrollTop <= target.clientHeight + 10;
    if (isAtBottom) {
      const nextGroupKeys = Tabs.slice(currentChunk * chunkSize, (currentChunk + 1) * chunkSize)
        .map((tab) => tab.key)
        .filter((key) => !visibleGroups.includes(key)); // Lọc nhóm chưa có trong `visibleGroups`

      if (nextGroupKeys.length) {
