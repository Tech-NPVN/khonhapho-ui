...
const FormSchema = z.object({
  name: z
    ...
    .refine((text) => text.trim().length > 5, 'Tên blog phải có ít nhất 6 ký tự.'),
  content: z
    .string({ message: REQUIRED_MSG_SAMPLE })
    .min(1, REQUIRED_MSG_SAMPLE)
...
/** (Admin) 'Index' Thêm/cập nhật blog */
      ...
      <Form className="w-full mt-3" form={form} onFinish={handleSubmit}>
        <div>
          <Form.Item<FormType>
            ...
            wrapperCol={{ span: 24 }}
          >
            <Input size="large" className="h-10" placeholder="Nhập tên blog" autoComplete="off" />
          </Form.Item>
          <Form.Item<FormType>
            name="content"
            rules={[rule]}
